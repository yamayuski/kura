...
Kura は、団体やグループのプライベートデータを自社管理のもとで LLM と協調する形で保存・整理・検索できるモダンな wiki サービスです。
...
| **ランタイム** | Bun |
| **言語** | TypeScript |
| **バックエンド** | Hono（軽量 Web フレームワーク） + PostgreSQL(+ pgvector) |
...
| 2 | インラインエディタ | 編集モードは存在せず、ユーザーがカーソルを置いた時点でその行が編集モードに移行。改行時に自動保存・コミット | **必須** |
...
| 項目 | 要件 | 備考 |
| --- | --- | --- |
| **スケーラビリティ** | 従業員数に応じた(数人～数千人)水平スケーリングに対応 | 組織規模の成長に柔軟に対応 |
